...
class CrossSectionSpace:
    def __init__(self, ipbsd, period_limits, fstiff, iteration=False, reduce_combos=True):
        """
        Initialize
    ...
    def read_solutions(self, cache_dir=None):
        """
        ...
        if cache_dir is not None:
            ...
            if not self.iteration and not cache_path.exists():
                ...
                self.solutions_x.to_csv(cache_path.parents[0] / "solution_cache_space_x.csv")
                self.solutions_y.to_csv(cache_path.parents[0] / "solution_cache_space_y.csv")
                self.solutions_gr.to_csv(cache_path.parents[0] / "solution_cache_space_gr.csv")

            # If solutions file exists, read and derive the solutions
            if cache_path.exists():
                self.solutions = pd.read_csv(cache_path, index_col=[0])
                self.solutions_x = pd.read_csv(cache_path.parents[0] / "solution_cache_space_x.csv", index_col=[0])
                self.solutions_y = pd.read_csv(cache_path.parents[0] / "solution_cache_space_y.csv", index_col=[0])
                self.solutions_gr = pd.read_csv(cache_path.parents[0] / "solution_cache_space_gr.csv", index_col=[0])
    ...
    def get_all_solutions(self):
        ...
        for i in self.elements.index:
            ...
            if self.check_target_t(periods[0], self.period_limits["1"]) and \
                ...
                solutions_x["T"].iloc[cnt] = periods[0]
                solutions_y["T"].iloc[cnt] = periods[1]
                solutions_x["Weight"].iloc[cnt] = weight
                solutions_y["Weight"].iloc[cnt] = weight
                solutions_x["Mstar"].iloc[cnt] = mstar[0]
                solutions_y["Mstar"].iloc[cnt] = mstar[1]
                solutions_x["Part Factor"].iloc[cnt] = gamma[0]
                solutions_y["Part Factor"].iloc[cnt] = gamma[1]
