...
def model_rXX(idacm,idacr,a,ac,mc,mr,mf,r,req,T,pw,filletstyle,N):
    ...
    for i in range(3): 
        ...
        if filletstyle==0:
            ...
            if mi<mf: R_rXX=np.exp((np.log(m_rXX)-b0[i])/b1[i])
            else: R_rXX=np.repeat(Rmr,len(m_rXX))           
            ...
            else: lRmi=np.log(Rmr)
            if lRmi<np.log(Rmr):
                if b1[i]>=slope_mXXend: slope_mXXend=b1[i]+0.05
                else: slope_mXXend=b1[i]-0.05
                int_mXXend=np.log(mr)-slope_mXXend * np.log(Rmr)
                lRmi=(int_mXXend-b0[i])/(b1[i]-slope_mXXend)            
            lmmi=b0[i]+b1[i]*lRmi
            new_mmi=np.exp(lmmi)            
...
def regions2model(a,mc,ac,r,mf,mr):
    if (mc>1) and (mf>1): pxx=1 
    else: pxx=0
    if (ac!=0) and (mf>mc) and (mr>mc): mxx=1  
    else: mxx=0
    if (r!=0) and (mf>mr): rxx=1 
    else: rxx=0
    return pxx,mxx,rxx
...
def spo2ida_allT(mc,a,ac,r,mf,T,pw,filletstyle=3,N=10):
    ...
    if pxx: [idacm,idacr,Rmc,slmc]=model_pXX(idacm,idacr,a,mc,T,pw,N)
    else: Rmc,slmc=[1,1,1],[1,1,1]
    if mxx: [idacm,idacr]=model_mXX(idacm,idacr,a,ac,mc,T,pw,mr,meq,mend,mpeak,mf,Rmc,slmc,filletstyle,N)
...
def spo2ida_spo(mc,a,ac,r,mf):
    ...
    if r==0 and mf>spocm[3]: spocr[3]=0
    else: spocr[3]=spocr[2]-(spocm[3]-spocm[2])*ac
    spocr[4]=r
    spocm[4]=mf
    return spocm,spocr
