...
class IPBSD:
    ...
    def run_ipbsd(self):
        ...
        data_to_store = {}
        for case in range(len(sols)):
            solution = sols.loc[case]
            solution, modes = ipbsd.get_all_section_combinations(t_lower=None, t_upper=None, solution=solution,
                                                                 data=ipbsd.data)
            part_factor, m_star, say, dy = self.iterate_phase_3(ipbsd, case_directory, solution, overstrength)
            phase_4 = self.iterate_phase_4(ipbsd, say, dy, sa, period_range, solution,
                                           modes, table_sls, t_lower, t_upper)
            forces = next(phase_4)
            demands = next(phase_4)
            details, hard_ductility, fract_ductility = next(phase_4)
            warn = next(phase_4)
            data_to_store[case] = {"warn": warn, "demands": demands, "details": details,
                                   "ductilities": [hard_ductility, fract_ductility], "modes": modes}
            if case == 100:
                break

        self.store_results(case_directory / "all", data_to_store, "pkl")
